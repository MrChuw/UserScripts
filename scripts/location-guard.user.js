...
// @updateURL       https://github.com/MrChuw/UserScripts/raw/main/scripts/location-guard.user.js
// @downloadURL     https://github.com/MrChuw/UserScripts/raw/main/scripts/location-guard.user.js
// @tag             privacy
// @tag             location
// @tag             spoof
// ==/UserScript==
    ...
    async function addNoise(position) {
        ...
        if (paused || level === "real");
            ...
            if (
                "level" in cachedPos &&
                cachedPos[level] &&
                (Date.now() - cachedPos[level].epoch) / 60000 < cachedPos[level].cacheTime
            ) {
        ...
        navigator.geolocation.watchPosition = function (cb1, cb2, opt) {
            ...
            (async () => {
                if (await isWatchAllowed()) {
                    const realId = watchPosition.call(
                        navigator.geolocation,
                        (pos) => cb1?.(pos),
                        (err) => cb2?.(err),
                        opt,
