...
(function () {
    ...
    function log(...args) {
        if (ENABLE_LOG) console.log('[Twitch Script]', ...args);
    }
    ...
    function isOnChannelPage() {
        if (location.hostname === 'player.twitch.tv') return true;
    ...
    function observeAndAlwaysRemove(selector, options = { removeParent: false, logLabel: selector }) {
        const removeAll = () => {
    ...
    function updateVolume(newVolume) {
        ...
        if (fill) fill.style.width = `${newVolume * 100}%`;
